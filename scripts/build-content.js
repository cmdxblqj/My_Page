...
function parseAudioFilename(filename) {
  ...
  if (lastDash > 0) {
    return {
      title: name.substring(0, lastDash).trim(),
      artist: name.substring(lastDash + 1).trim()
    };
  }
...
function scanAudioFiles() {
  ...
  for (const file of files) {
    ...
    tracks.push(track);
    console.log(`  ✓ audio/${file} → "${title}" by ${artist}`);
  }
...
function generateDataJs(audioTracks, archiveArticles, codexArticles, galleryImages) {
  ...
  lines.push(`// Audio   (Music Player): ${audioCount} tracks from audio/`);
  ...
  lines.push('// ⚠️  DO NOT EDIT THIS FILE MANUALLY — edit the source files instead.');
  lines.push('');
  lines.push('/* ═══════════════════════════════════ AUDIO PLAYLIST (Music Player) ═══════════════════════════════════ */');
  lines.push('var AUDIO_PLAYLIST = [');
  if (audioTracks.length === 0) {
    lines.push('  // No tracks yet. Add .mp3 files to audio/');
  } else {
    audioTracks.forEach((t, i) => {
      lines.push(`  { title: ${formatJsString(t.title)}, artist: ${formatJsString(t.artist)}, src: ${formatJsString(t.src)} }`);
      if (i < audioTracks.length - 1) lines.push(',');
    });
  }
  lines.push('];');
...
function main() {
  ...
  // Scan audio files
  console.log('🎵 Audio (Music Player):');
  const audioTracks = scanAudioFiles();
  console.log(`   → ${audioTracks.length} track(s)\n`);
  ...
  // Generate output
  const output = generateDataJs(audioTracks, archiveArticles, codexArticles, galleryImages);
  fs.writeFileSync(OUTPUT_FILE, output, 'utf-8');

  console.log(`✅ Generated: ${path.relative(ROOT, OUTPUT_FILE)}`);
  console.log(`   Audio: ${audioTracks.length} | Archive: ${archiveArticles.length} | Codex: ${codexArticles.length} | Gallery: ${galleryImages.length}\n`);
}
